...
export const problemSchema = z.object({
  ...
  stateLimit: z.number({ coerce: true }).int().min(1).max(100),
  depthLimit: z.number({ coerce: true }).int().min(1).max(1000000),
  maxStepLimit: z.number({ coerce: true }).int().min(1).max(1000000),
...
export type UpdateProblemSchema = z.infer<typeof updateProblemSchema>;
