...
import { BaseState, TransitionData } from "./BaseState";

export abstract class BaseDesigner {
  abstract states: Map<number, BaseState>;
  abstract alphabet: string[];

  abstract stateToIndex: Map<string, number>;
  ...
  getTransition(from: number, to: number): TransitionData[] {
    ...
    const symbols = state.transitions.get(to);
    return symbols ?? [];
  }
